...
int main()
{
    ...
    for (int i = 1; i < t_filename.size(); i++)
    {
        ...
        matchedEntitiesList.push_back(matchedEntities1);

        cout << "First Level Matching for Sequence " << i + 1 << ":" << endl;
        ...
        writeDecompressedSequenceToFile("decompressed_sequence" + to_string(i) + ".fa", decompressedSequence);

        // Output decompressed sequence for verification
        cout << "Decompressed Sequence: " << decompressedSequence << endl;
    }

    // Perform second level matching
    secondLevelMatching(matchedEntitiesList, matchedEntitiesList, 3);

    // Output second level matching results
    cout << "Second Level Matching Results:" << endl;
    for (const auto &matchedEntities : matchedEntitiesList)
    {
        for (const auto &entity : matchedEntities)
        {
            cout << "Position: " << entity.position << ", Length: " << entity.length << ", Mismatched: " << entity.misMatched << endl;
        }
    }

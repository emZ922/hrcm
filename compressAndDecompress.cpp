...
void writeEncodedDataToFile(const vector<int> &encodedData, const vector<pair<int, char>> &encodedSpecialChars, const vector<CharInfo> &nList, const vector<LowercaseChar> &lowercaseList, const string &referenceSequence, const string &filename) {
...
void readEncodedDataFromFile(const string &filename, vector<int> &encodedData, vector<pair<int, char>> &encodedSpecialChars, vector<CharInfo> &nList, vector<LowercaseChar> &lowercaseList, string &referenceSequence) {
    ...
    inFile.read(reinterpret_cast<char*>(encodedData.data()), size * sizeof(int));
...
void decodeSequenceInformation(const vector<int> &encodedData, vector<Entity> &matchedEntities) {
    ...
    for (size_t i = 0; i < encodedData.size(); i += 2) {
        ...
        entity.misMatched = ""; // Placeholder if mismatched info is used
...
void decompressData(const string &encodedFilename, string &decompressedSequence) {
    ...
    for (const auto &special : encodedSpecialChars) {
        decompressedSequence[special.first] = special.second;
    }
}
...
int main() {
    ...
    writeEncodedDataToFile(encodedData, encodedSpecialChars, t_nList, r_lowercaseList, r_seq, "compressed_data.bin");
